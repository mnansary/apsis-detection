...
def memoBottomFunc(
    ...
    def processText(dataText,
      ...
      for i, p in enumerate(dataText):
        ...
        rem_str=p
        # find pure bangla words and numbers
        words=regex.findall(r"[^\x20-\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7C\x7E]+",p)
        for idx,word in enumerate(words):
          ...
          if prev_str.strip():
            comps+=[c for c in prev_str]
          # _data.append(word)   ### <<<<===
          if any(char in bangla_num+["_"] for char in word):
            comps+=[g for g in word]
          else:
            comps+=gp.word2grapheme(word)
...
#     h_max+=32
    ...
    for img in imgs_mdl:
        img=padImg(img, h_max, w_max)
#         cv2.resize(img_line_, (w, h//2), fx=0,fy=0, interpolation = cv2.INTER_NEAREST)
